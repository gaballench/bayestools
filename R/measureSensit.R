...
#'
#' @param ... Further arguments to pass to the graphical functions such as \code{lines} and \code{plot} internally, such as main, xlim, ylim, xlab, ylab, etc.
#'
...
#'
#' @details Sensitivity is measured as the overlapping portion between two densities following Ballen (in prep). It has a value between 0 and 1. The values of the vector rawData determine the behavior of the function and therefore attention must be paid to their consistence with the nature of arguments d1 and d2. Despite the function was designed in order to allow to quantify posterior sensitivity to the prior, this can be used to quantify any overlapping between two given densities and for any other purpose.
#' @examples
#' \dontrun{
...
#' below <- measureSensit(d1 = rnorm(1000000, mean = 0, 1), d2 = rnorm(1000000, mean = 0, 1), main = "Comp. dependence", colors = colors)
#' legend(x = "topright", legend = round(below, digits = 2))
#' # Sensitivity in two distributions partially overlapping
#' below <- measureSensit(d1 = rnorm(1000000, mean = 3, 1), d2 = rnorm(1000000, mean = 0, 1), main = "Partial dependence", colors = colors)
#' legend(x = "topright", legend = round(below, digits = 2))
#' # Sensitivity in two completely-different distributions
#' below <- measureSensit(d1 = rnorm(1000000, mean = 8, 1), d2 = rnorm(1000000, mean = 0, 1), main = "Comp. independence", colors = colors)
#' legend(x = "topright", legend = round(below, digits = 2))
#' # Don't plot, just return the intersection
#' measureSensit(d1 = rnorm(1000000, mean = 3, 1), d2 = rnorm(1000000, mean = 0, 1), plot = FALSE)
#' }
#' @export 
